...
class Auth @Inject() (
  ...
  def changePasswordForm(implicit messages: Messages) = Form(tuple(
    ...
    "password1" -> passwordValidation,
    "password2" -> nonEmptyText
  ) verifying (messagesApi("auth.passwords.notequal")(messages.lang), passwords => passwords._3 == passwords._2))
  ...
  def handleChangePassword = SecuredAction.async { implicit request =>
    // pretty awkward, but perhaps someone can explain Play 2.6's i18n changes
